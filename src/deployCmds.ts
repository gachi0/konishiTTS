...
import { allImport, ICommand, setting } from "./bot";
...
+ async function () {
    ...
    if (process.argv[2] === "guild") {
        await rest.put(Routes.applicationGuildCommands(clientId, setting.guildId), {
            body: (await allImport("commands") as ICommand[]).map(c => c.data.toJSON())
        });
        console.log(`${setting.guildId}にコマンドを登録しました！`);
    }
        ...
        await rest.put(Routes.applicationGuildCommands(clientId, setting.guildId), { body: [] });
        console.log(`${setting.guildId}からコマンドを削除しました！`);
    }
