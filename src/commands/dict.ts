...
//     let guild = await db.kGuild.upsert({
//       where: { id: intr.guildId }, // なかったら作成
//       update: { id: intr.guildId }, // https://stackoverflow.com/questions/71524243/how-to-find-or-create-a-record-with-prisma
//       create: { id: intr.guildId },
//       include: { words: true }
//     });
...
//     let nowPage = 1;
//     /**  総ページ数 */
//     const pageCount = () => Math.floor(guild.words.length / 25) + 1;
//     /** ページに表示する単語の一覧を出す関数 */
//     const nowPageDict = () => guild.words.slice((nowPage - 1) * 25, nowPage * 25);
...
//         // 単語をDBに登録
//         const word = modalResult.fields.getTextInputValue("dictWord");
//         const yomi = modalResult.fields.getTextInputValue("dictYomi");
//         await db.kWord.create({
//           data: { guildId: guild.id, word, yomi }
//         });

//         // 結果を報告
//         await modalResult.reply({
//           embeds: [
//             { title: "単語を登録しました", description: `単語: \`${word}\`\n読み: \`${yomi}\`` }]
//         });
//         // 辞書に反映
...
//               maxValues: guild.words.length,
//               options: guild.words.map(d => ({ label: `${d.word}: ${d.yomi}`, value: d.id.toString() }))
//             },]
//           },
//           {
//             type: ComponentType.ActionRow, components: [{
...
//         const delWords = guild.words.filter(d => delSelectResult.values.includes(d.id.toString()));

//         if (
//           !delWords.every(w => w.kUserId === intr.user.id)
//           &&
//           !intr.member.permissions.has(PermissionsBitField.Flags.Administrator)
//         ) {
...
//         // 辞書から単語を削除
//         await db.kWord.deleteMany({
//           where: { id: { in: delSelectResult.values } }
//         });
